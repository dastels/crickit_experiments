...
uint8_t convert_adc_pin(uint8_t pin)
{
  switch(pin) {
...
*  @param		mode the mode to set the pins to. One of SS_INPUT, SS_OUTPUT, or SS_INPUT_PULLUP.
*
*  @return     none
****************************************************************************************/
...
int8_t convert_pwm_pin(uint8_t pin)
{
  switch(pin) {
  case PWM_0_PIN:
    return 0;
...
*  @return     none
****************************************************************************************/
void Adafruit_seesaw::write8(uint8_t regHigh, uint8_t regLow, uint8_t value)
{
                           ...
                           uint8_t *buf, uint8_t num, uint16_t delay)
{
  ...
  for (int i = 0; i < num; i++) {
    buf[pos] = wiringPiI2CRead(_fd);
    // _i2cbus->read();
#ifdef SEESAW_I2C_DEBUG
    printf("0x%02u,", buf[pos]);
#endif
    pos++;
  }
#ifdef SEESAW_I2C_DEBUG
...
void Adafruit_seesaw::write(uint8_t regHigh, uint8_t regLow, uint8_t *buf, uint8_t num)
{
  ...
  wiringPiI2CWrite(_fd, regLow);
  for (int i = 0; i < num; i++) {
    wiringPiI2CWrite(_fd, buf[i]);
  }
#ifdef SEESAW_I2C_DEBUG
  printf("I2C write 0x%02u%02u : ", regHigh, regLow);
  for (int i = 0; i < num; i++) {
    if (i > 0) {
      printf(",");
    }
